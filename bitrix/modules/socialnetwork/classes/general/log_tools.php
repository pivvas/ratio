...
class CSocNetLogTools
{
	...
	{
		static $arForumCache = array();

		$res = false;

		if (
			!empty($arParams)
			&& !empty($arParams["COMMENT_SOURCE_ID"])
			&& intval($arParams["COMMENT_SOURCE_ID"]) > 0
			&& !empty($arParams["LOG_SOURCE_ID"])
			&& intval($arParams["LOG_SOURCE_ID"]) > 0
			&& CModule::IncludeModule('forum')
		)
		{
			if (!empty($arForumCache[$arParams["LOG_SOURCE_ID"]]))
			{
				$forumId = $arForumCache[$arParams["LOG_SOURCE_ID"]];
			}
			else
			{
				$arForumMessage = CForumMessage::GetByID($arParams["COMMENT_SOURCE_ID"]);
				$forumId = $arForumCache[$arParams["LOG_SOURCE_ID"]] = $arForumMessage["FORUM_ID"];
			}
	...
	public static function getDateTimeFormatted($timestamp, $arFormatParams)
	{
		...
		return (
			...
			&& strcasecmp(LANGUAGE_ID, 'DE') !== 0
				? ToLower(FormatDate($arFormat, $timestamp))
