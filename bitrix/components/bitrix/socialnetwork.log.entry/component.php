...
if ($arEvent)
{
	...
	)
	{
		...
		$arEventMeta = CSocNetLogTools::FindLogEventByID($arEvent["EVENT"]["EVENT_ID"]);
		if (
			!empty($arEventMeta)
		...
		{
			foreach ($arCommentsFullListCut as $key => $arCommentTmp)
			{
				$arCommentsFullListCut[$key]["CAN_EDIT"] = call_user_func($arEventMeta["COMMENT_EVENT"]["METHOD_CANEDIT"], array(
					"LOG_SOURCE_ID" => $arEvent["EVENT"]["SOURCE_ID"],
