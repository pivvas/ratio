		...
		)
		{
			...
			foreach($arResult["arComments"] as $key => $arCommentTmp)
			{
				...
				if (
					!empty($arCommentEvent)
					&& !empty($arCommentEvent["METHOD_CANEDIT"])
					&& !empty($arCommentTmp["EVENT"]["SOURCE_ID"])
					&& intval($arCommentTmp["EVENT"]["SOURCE_ID"]) > 0
					&& !empty($arLog["SOURCE_ID"])
					&& intval($arLog["SOURCE_ID"]) > 0
				)
				{
					$canEdit = call_user_func($arCommentEvent["METHOD_CANEDIT"], array(
						"LOG_SOURCE_ID" => $arLog["SOURCE_ID"],
						"COMMENT_SOURCE_ID" => $arCommentTmp["EVENT"]["SOURCE_ID"]
					));
				}
				else
				{
	...
	elseif ($action == "get_comment_src")
	{
		...
		)
		{
			...
			$dbRes = CSocNetLogComments::GetList(
				...
				false,
				false,
				array("ID", "EVENT_ID", "MESSAGE", "USER_ID", "SOURCE_ID", "LOG_SOURCE_ID")
			);
			...
			{
				$arCommentEvent = CSocNetLogTools::FindLogCommentEventByID($arRes["EVENT_ID"]);
				if (
					!empty($arCommentEvent)
					&& !empty($arCommentEvent["METHOD_CANEDIT"])
					&& intval($arRes["SOURCE_ID"]) > 0
					&& intval($arRes["LOG_SOURCE_ID"]) > 0
				)
				{
					$canEdit = call_user_func($arCommentEvent["METHOD_CANEDIT"], array(
						"LOG_SOURCE_ID" => $arRes["LOG_SOURCE_ID"],
						"COMMENT_SOURCE_ID" => $arRes["SOURCE_ID"]
					));
				}
				else
				{
